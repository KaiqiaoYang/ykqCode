...
MyStruct MergeSort(MyStruct mystruct,int p,int r)
{
	if (p < r)
	{
		int q = (p + r) / 2;
		mystruct = MergeSort(mystruct, p, q);
		mystruct = MergeSort(mystruct, q + 1, r);
		mystruct = Merge(mystruct, p, q, r);
	}
	return mystruct;
}
...
void main()
{
	...
	mystruct = MergeSort(mystruct, 0, 9);
	for (int i = 0; i <= 9; i++)
	{
		cout << mystruct.A[i] << " ";
	}
